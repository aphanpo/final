                ...
                <div className="mainShelterList">
                    {shelters.map(shelter => (
                        <div className="shelterList">
                            ...
                            <p><Icon icon="bed"></Icon> Available: {shelter.open_beds} out of {shelter.total_beds} </p>
                            <p className="last">Meals Available: {shelter.meal_option} <button onClick={e => bed_count(shelter.id)} className="resButton"><Link to={{
                                pathname: `/Reservation/${shelter.name}/${shelter.id}`,
                                state: {
                                    open_beds: shelter.open_beds
                                }
                            }}> Get me on the list</Link></button></p>        
                        </div>   
                    ))}
                </div>
